...
def refresh(path):
    x,y = wait_until_image_located_on_screen("images/refresh.jpg",duration=8)
    pyautogui.click(x,y)
    time.sleep(.3)

def make_payment(creditcardname,creditcardnumber,creditcardexpirydate,securitycode):
    x4,y4 = wait_until_image_located_on_screen("images/continuepayment.jpg",duration=20)
    pyautogui.click(x4,y4)

    zoomOut("images/optionsdarkblue.jpg",number_of_zooms=4)

    x2,y2 = wait_until_image_located_on_screen("images/continue.jpg",duration=20)
    pyautogui.click(x2,y2)
    
    x3,y3 = wait_until_image_located_on_screen("images/creditcardbutton.jpg",confidence=0.95,duration=10)
    ...
    locate_and_click("images/creditcardname.jpg",duration=0.01,tween=None)
    pyautogui.typewrite(creditcardname)

    #click the input button that allows you to enter your credit card number
    locate_and_click("images/creditcardnumber.jpg",duration=0.01,tween=None)
    pyautogui.typewrite(creditcardnumber)

    #click the input button that allows you to enter the expiry date
    locate_and_click("images/expirydate.jpg",duration=0.01,tween=None)
    pyautogui.typewrite(creditcardexpirydate)
    
    #click the input button that allows you to enter your CV number
    locate_and_click("images/securitycode.jpg",duration=0.01,tween=None)
    pyautogui.typewrite(securitycode)
    
    #agree to the terms and conditions
    locate_and_click("images/agreetoterms.jpg",duration=0.01,tween=None,confidence=0.95)
...
if __name__=="__main__":
    ...
    while not(keyboard.is_pressed("esc")):
        try:
            locate_and_click("images/keyword.jpg",grayscale=True)
            print("Success!")
        except:
            #refresh the page
            ...
            time.sleep(1)
            try:
                locate_and_click("images/cross.png",grayscale=True)
                time.sleep(0.2)
            except:
                pass

            locate_and_click("images/shopping_cart.jpg",grayscale=True)
